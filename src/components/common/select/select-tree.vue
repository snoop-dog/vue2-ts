...
<template>
  <div class="sitebox form-sitebox" v-clickoutside="handleClose">
    <div class="Collection_site el-select" @click.stop="showTreeData">
      <div
        v-if="siteTreeNamesArray.length > 0 && !showTreeFlag"
        class="el-select__tags"
      >
        <span>
          <span
            v-for="(item, index) in siteTreeNamesArray"
            :key="index"
      ...
      <input
        v-else
        style="min-width: 20px; right: 0px"
        autocompelete="off"
        :placeholder="placeholder"
        class="el-select__input is-undefined"
        debounce="0"
        v-model="filterText"
      />
    </div>
    ...
    <div v-if="showTreeFlag" class="sitetree" @click.stop>
      <el-tree
        ...
        :default-expanded-keys="expandedKeys ? expandedKeys : siteTreeCodes"
        :default-checked-keys="siteTreeCodes"
        class="filter-tree"
        :data="siteTreeData"
        :props="defaultProps"
...
export default {
  ...
  computed: {
    siteTreeNamesArray () {
      if (
        this.siteTreeData.length > 0 &&
        typeof this.siteTreeCodes === 'object' &&
        this.siteTreeCodes.length > 0 &&
        this.defaultProps
      ) {
        const ary = []
        this.siteTreeCodes.map((item) => {
          getArray(
            this.siteTreeData,
            item,
  ...
  methods: {
     ...
     * @desc 节点树控件的change事件
     */
    handleCheckChange: function (indeterminate) {
      if (this.isLeaf) {
        const valueAry = []
        const parantAry = this.siteTreeData.map(
          (item) => item[this.defaultProps.id]
        )
        const tt = indeterminate.checkedNodes
        tt.map((item) => {
        ...
        const new_arr = valueAry.filter(
          (x) => !isSpliceAry.some((item) => x === item)
        )
        this.$emit('change', new_arr)
        this.$emit('input', new_arr)
      } else {
        const tt1 = this.parentid
          ? this.$refs.tree2.getCheckedNodes()
          : this.$refs.tree2.getCheckedNodes(true)
        const valueAry = []
        tt1.map((item) => {
          valueAry.push(item[this.defaultProps.id])
        })
