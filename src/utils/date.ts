 ...
 * @LastEditTime: 2021-05-05 19:53:18
...
export function doDateTimeShift (arg1, arg2, arg3) {
  ...
  function sliceDateString (val) {
    ...
    // if (val.indexOf(str)) {
    //   const finalStr = val.replace(new RegExp(str, 'g'), '')
    //   return finalStr
    // } else {
    //   return val
    // }
  ...
  if (arg.length <= 1) {
    if (typeof (arg[0]) === 'string') {
      if (arg[0].indexOf('yyyy') !== -1) Format(new Date(), 'yyyy-MM-dd HH:mm:ss')
        ...
        const date = Format(new Date(Date.parse(arg1.replace(/-/g, '/'))), 'yyyy-MM-dd HH:mm:ss')
        return sliceDateString(date)
      }
    }
    if (typeof (arg[0]) === 'number') {
      const date = Format(new Date(arg1), 'yyyy-MM-dd HH:mm:ss')
      return sliceDateString(date)
    }
  ...
  if (arg.length <= 2) {
    if (typeof (arg[0]) === 'string') {
      const date = Format(new Date(Date.parse(arg[0].replace(/-/g, '/'))), arg2)
      return sliceDateString(date)
    }
    if (typeof (arg[0]) === 'number') {
      const date = Format(new Date(arg1), arg2)
      return sliceDateString(date)
    }
    if (typeof (arg[0]) === 'object') {
      const date = Format(new Date(arg1), arg2)
